...
void BotControl::trueCallBack(const gazebo_msgs::LinkStates& msg){ //an array containing all the messages
    // get index
    int i = 0;
    for (; i<msg.name.size(); ++i) //iterate through the array and select base_link message
        if (msg.name[i] == "husky::base_link")
            break;
    if (i >= msg.name.size())
        return; // does not exist

    //We are taking the the reference point of the robot (Origin P)
    geometry_msgs::Pose pose = msg.pose[i]; //http://docs.ros.org/en/noetic/api/geometry_msgs/html/msg/Pose.html
